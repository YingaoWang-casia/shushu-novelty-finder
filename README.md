...
> [!IMPORTANT]
> **Alpha 发布状态（v0.2.0a1）**：工程化运行时已经公开，主分支通过 Python 3.9 /
> 3.12 CI，但这不是“效果优于 baseline”的稳定版声明。固定 60-seed × 4-system
> 运行已经完成；两名真实科研评审者的盲评、一致性统计和可发布聚合报告仍待完成。
> 在这些证据公开前，请把本版本视为可复现工程 Alpha，不要把自动指标或示例输出
> 解读为比较效果结论。完整状态见[工程验收审计](docs/acceptance-audit.md)。
